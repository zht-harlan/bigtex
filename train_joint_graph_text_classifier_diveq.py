...
def prepare_loaders(data, batch_size, num_layers):
    ...
    train_loader = NeighborLoader(data, num_neighbors=num_neighbors, batch_size=batch_size, input_nodes=data.train_idx)
    valid_loader = NeighborLoader(data, num_neighbors=num_neighbors, batch_size=batch_size, input_nodes=data.valid_idx)
    test_loader = NeighborLoader(data, num_neighbors=num_neighbors, batch_size=batch_size, input_nodes=data.test_idx)
...
def build_summary(run_results, dataset_name, backbone_name):
    ...
    summary = {
        ...
        "model": "JointGraphTextClassifierDiVeQ",
...
def export_node_codes(model, loader, refined_texts, device, output_csv_path):
    ...
    print(f"Saved DiVeQ node codes to: {output_csv_path}")
...
def main():
    ...
    if cached_embeddings.size(0) != data.num_nodes:
        raise ValueError(f"Embedding row count {cached_embeddings.size(0)} does not match num_nodes {data.num_nodes}.")
    if len(refined_texts) != data.num_nodes:
        raise ValueError(f"Refined text count {len(refined_texts)} does not match num_nodes {data.num_nodes}.")
    ...
    runs_csv_path = os.path.join(dataset_results_dir, f"{dataset_name}_diveq_joint_runs.csv")
    summary_csv_path = os.path.join(dataset_results_dir, f"{dataset_name}_diveq_joint_summary.csv")
    checkpoint_path = os.path.join(dataset_results_dir, f"{dataset_name}_diveq_joint_model.pt")
    ...
    if best_model_state is not None and best_test_loader is not None:
        best_model = CrossModalFusionDiVeQPLM(
            input_dim=input_dim,
            hidden_dim=args.hidden_dim,
            num_classes=num_classes,
            gnn_type=args.gnn_type,
        ...
        export_node_codes(best_model, best_test_loader, refined_texts, device, args.node_codes_path)
