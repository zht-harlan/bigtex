...
def evaluate(model, loader, refined_texts, device, split_name):
    model.eval()
    total_loss = 0.0
    y_true_all = []
    y_pred_all = []
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"Evaluating [{split_name}]"):
            ...
            loss = F.cross_entropy(outputs["logits"], seed_labels)
...
def train_one_run(
    ...
    for epoch in range(epochs):
        ...
        val_metrics = evaluate(model, valid_loader, refined_texts, device, "val")
        test_metrics = evaluate(model, test_loader, refined_texts, device, "test")
...
def build_summary(run_results, dataset_name, backbone_name):
    ...
    summary = {
        ...
        "model": "JointGraphTextClassifier",
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Exporting node codes [test]"):
            ...
            for node_id, label, codes in zip(seed_node_ids, seed_labels, seed_codes):
                row = {"node_id": int(node_id), "label": int(label)}
...
def main():
    ...
    parser.add_argument("--results_dir", default="joint_fusion_results", help="output directory")
    ...
    runs_csv_path = os.path.join(dataset_results_dir, f"{dataset_name}_joint_runs.csv")
    summary_csv_path = os.path.join(dataset_results_dir, f"{dataset_name}_joint_summary.csv")
    checkpoint_path = os.path.join(dataset_results_dir, f"{dataset_name}_joint_model.pt")
