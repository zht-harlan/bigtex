...
def resolve_backbone_name(model_name):
    return BACKBONE_ALIASES.get(model_name.lower(), model_name)


def infer_lora_targets(backbone_name):
    lowered = backbone_name.lower()
    if "deberta" in lowered:
        return ["query_proj", "key_proj", "value_proj"]
    if "roberta" in lowered or "bert" in lowered or "scibert" in lowered:
        return ["query", "key", "value"]
    return ["query", "key", "value"]


class CrossModalFusionPLM(nn.Module):
    def __init__(
        self,
        input_dim,
        hidden_dim,
        ...
        self.graph_encoder = QuantizedPurifiedGraphEncoder(
            input_dim=input_dim,
        ...
        struct_dim = quantizer_dim or hidden_dim
        self.struct_token_projection = nn.Linear(struct_dim, self.plm_hidden_size)
        self.level_embeddings = nn.Embedding(num_quantizers, self.plm_hidden_size)
    ...
    def forward(self, x, edge_index, texts, batch_size=None):
        ...
        struct_tokens = self.struct_token_projection(quantized_stack)
        level_ids = torch.arange(struct_tokens.size(1), device=device).unsqueeze(0).expand(batch_size, -1)
        struct_tokens = struct_tokens + self.level_embeddings(level_ids)
        sep_embedding = self._get_sep_embedding(batch_size, device)

        fused_embeddings = torch.cat([struct_tokens, sep_embedding, text_embeddings], dim=1)
        struct_mask = torch.ones((batch_size, struct_tokens.size(1)), dtype=tokens["attention_mask"].dtype, device=device)
        ...
        if self.debug_shapes and not self.shape_debug_printed:
            print(f"Struct token shape: {tuple(struct_tokens.shape)}")
        ...
        return {
            ...
            "residual_states": graph_outputs["residual_states"],
